    ...
    <div className="popup-modal">
      ...
      >
        {/* <div className={ `${ trigger ? 'popup-modal-container-show' : 'popup-modal-container-hide'}`}>
              {exitIcon ? <img className='exit-icon-show' onClick={() => {setTrigger(false)}} src={XMark} alt='close popup' /> : <></>}
              
            </div> */}
      </div>

      <div className={` ${trigger ? 'popup-modal-container-show' : 'popup-modal-container-hide'}`}>
        {/* option of exit button */}
        {exitIcon ? (
          <img
      ...
      {/* first start with background -- fills full screen and has blur options
    {
      trigger ? <></> : <></>
    }
