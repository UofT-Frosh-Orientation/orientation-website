...
const ScuntController = {
  ...
  async login(req, res, next) {
    try {
      ...
      req.log.info({msg: "Successful Scunt Login by user " + existingUser.id , user: existingUser.getResponseObject()});
      return res.status(200).send({ ...userInfo });
    } catch (err) {
      req.log.fatal({msg: "User Scunt Login Failure: user " + existingUser.id, err, user: existingUser.getResponseObject()});
      next(err);
    }
  },
